...
$sanifilho = "SELECT * FROM rfa_socios_filhos INNER JOIN rfs_socios ON rfs_socios.ref_socio = rfa_socios_filhos.id_socio WHERE rfs_socios.clube='$clube' AND rfs_socios.status_socio=1 ORDER BY DAY(data_nascto_filho) ASC";
...
foreach($pegaadmiss as $pegaadmi){
	...
	if($intervalo->y == 0){
		...
		$interval = $intervalo->y." anos de associado(a)";
	}
